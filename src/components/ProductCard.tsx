...
function ProductCard({ data }: { data: ProductCardProps }) {
  ...
  return (
    <div className={'bg-white relative mb-3 p-5 flex flex-col shadow-lg justify-between'}>
      {data.discount > 0 && (
        <div className='flex'>
          <div className="absolute z-10 bg-red-600 text-white font-semibold mt-3 top-0 left-0 px-4">
            {t("discount")}
          </div>
            <div className='absolute z-10 border-2 text-red-600 border-red-600 font-semibold mt-3 top-0 right-3 px-4'>
             <del className='font-bold'>{data.price} m</del>
            </div>
        </div>
      ...
      <div className='py-2 text-lg font-semibold hover:text-custom-blue'>{data.name}</div>
      {data.inStock ? (
      ...
      <div className='flex items-center justify-between mt-2'>
        ...
        <div className='flex items-center'>
          <button
            className="bg-orange-400 hover:bg-orange-500 transition-all duration-200 p-2 rounded-full">
            <BiBasket size={26} className='text-white font-bold' />
          </button>
          <button
            className="bg-orange-400 hover:bg-orange-500 transition-all duration-200 p-2 rounded-full ml-2">
